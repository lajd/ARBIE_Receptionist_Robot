# The whole file was taken from @davidsandberg implementation
# https://github.com/davidsandberg/facenet/blob/master/facenet/src/facenet.py
...
def inception(inp, inSize, ks, o1s, o2s1, o2s2, o3s1, o3s2, o4s1, o4s2, o4s3, poolType, name, phase_train=True,
              use_batch_norm=True):
    print('name = ', name)
    ...
    with tf.name_scope(name):
        if o1s > 0:
            conv1 = conv(inp, inSize, o1s, 1, 1, 1, 1, 'SAME', 'in1_conv1x1', phase_train=phase_train,
                         use_batch_norm=use_batch_norm)
            net.append(conv1)

        if o2s1 > 0:
            conv3a = conv(inp, inSize, o2s1, 1, 1, 1, 1, 'SAME', 'in2_conv1x1', phase_train=phase_train,
                          use_batch_norm=use_batch_norm)
            conv3 = conv(conv3a, o2s1, o2s2, 3, 3, ks, ks, 'SAME', 'in2_conv3x3', phase_train=phase_train,
                         use_batch_norm=use_batch_norm)
            net.append(conv3)

        if o3s1 > 0:
            conv5a = conv(inp, inSize, o3s1, 1, 1, 1, 1, 'SAME', 'in3_conv1x1', phase_train=phase_train,
                          use_batch_norm=use_batch_norm)
            conv5 = conv(conv5a, o3s1, o3s2, 5, 5, ks, ks, 'SAME', 'in3_conv5x5', phase_train=phase_train,
                         use_batch_norm=use_batch_norm)
            net.append(conv5)
        ...
        if o4s2 > 0:
            pool_conv = conv(pool, inSize, o4s2, 1, 1, 1, 1, 'SAME', 'in4_conv1x1', phase_train=phase_train,
                             use_batch_norm=use_batch_norm)
        else:
            pool_conv = pool
    ...
    for i in xrange(people_per_batch):
        ...
        for j in range(1, n):
            ...
            while sel_neg_idx >= emb_start_idx and sel_neg_idx <= emb_start_idx + n - 1:
                sel_neg_idx = (np.random.randint(1,
                                                 2 ** 32) % nrof_images) - 1  # Seems to give the same result as the lua implementation
...
def calculate_roc(thresholds, embeddings1, embeddings2, actual_issame, seed):
    ...
    for fold_idx, (train, test) in enumerate(folds):
        ...
        for threshold_idx, threshold in enumerate(thresholds):
            tprs[fold_idx, threshold_idx], fprs[fold_idx, threshold_idx], _ = calculate_accuracy(threshold, dist[test],
                                                                                                 actual_issame[test])
